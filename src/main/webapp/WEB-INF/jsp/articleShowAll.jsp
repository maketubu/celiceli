...
<body id="nv_forum" class="pg_forumdisplay" onkeydown="if(event.keyCode==27) return false;">
	...
	<div id="hd">
		<div class="wp">
					...
					<div class="well well-small pull-right user-panel-mini">
						...
						<c:choose>
							<c:when test="${sessionScope.userName==''||sessionScope.userName==null}">
								<form action="/loginService.action" method="post">
			...
			<div id="nv">
				<ul class="nav ">
							<li class="active " id="mn_forum "><a href="/articleIndex.action"
				...
				<div id="threadlist" class="btr bbr tl bm bmw trans-80">
					...
					<c:forEach items="${article_list}" var="list">
								<tbody id="normalthread_555837">
									<tr>
										<td class="icn"><a href="/articlePage.action?articleID=${list.articleID}" title="有新回复 - 新窗口打开"
										...
										<th class="new"><a class="tdpre y"
											...
											<a href="/articlePage.action?articleID=${list.articleID}" onclick="atarget(this)" class="s xst">${list.articleTitle }</a> <span class="tps">&nbsp;<a
												href="/articlePage.action?articleID=${list.articleID}"></a></span></th>
										<td class="by"><cite> <a href="/articlePage.action?articleID=${list.articleID}" c="1"
												style="color: #333333;">${list.articleUserName }</a></cite> <em><span>${list.articleTime }</span></em>
										</td>
										<td class="num"><a href="javascript:void(0);" class="xi2">${list.articlePageView}</a></td>
										<td class="by"></td>
									</tr>
								</tbody>
								...
								$("#butt").click(function(){
										...
										if(admin=="null"){
											...
											if(biaoti==null||biaoti==""||neirong==null||neirong==""){
												...
												var html= "";
												//将得到的使用ajax提交
												$.post("/articlePublish.action",{"title":biaoti,"content":neirong},function(test){
													var ht=eval("("+test+")");
